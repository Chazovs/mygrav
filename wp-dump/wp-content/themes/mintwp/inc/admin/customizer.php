...
function mintwp_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('mintwp_main_options_panel', array( 'title' => esc_html__('Theme Options', 'mintwp'), 'priority' => 10, ));
    endif;
    
    $wp_customize->get_section( 'title_tagline' )->panel = 'mintwp_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'colors' )->panel = 'mintwp_main_options_panel';
    ...
    mintwp_getting_started($wp_customize);
    mintwp_header_options($wp_customize);
    mintwp_post_options($wp_customize);
    mintwp_social_profiles($wp_customize);
    mintwp_footer_options($wp_customize);
    mintwp_other_options($wp_customize);
    mintwp_recomm_plugin_options($wp_customize);
    mintwp_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'mintwp_register_theme_customizer' );

function mintwp_customizer_js_scripts() {    
    wp_enqueue_script('mintwp-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'mintwp_customizer_js_scripts' );