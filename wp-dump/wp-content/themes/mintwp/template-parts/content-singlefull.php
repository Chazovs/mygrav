                ...
                if ( !(mintwp_get_option('hide_thumbnail')) ) {
                    if ( !(mintwp_get_option('hide_thumbnail_single')) ) {
                        if ( mintwp_get_option('thumbnail_link') == 'no' ) {
                            the_post_thumbnail('mintwp-large-image', array('class' => 'mintwp-post-thumbnail-single'));
                        } else { ?>
                            <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'mintwp' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('mintwp-large-image', array('class' => 'mintwp-post-thumbnail-single')); ?></a>
                <?php   }
                    }
                }
            }
